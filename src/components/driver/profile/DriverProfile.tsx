...
import { useProfile, useUpdateProfile, useAddVehicle, useRemoveVehicle } from '@/hooks/useProfile'
...
export function DriverProfile({ userId, onBack }: ReadonlyDriverProfileProps) {
  const { data: user, vehicles, vehicleTypes, isLoading } = useProfile(userId)
  const { mutate: updateProfile, isPending: isUpdating } = useUpdateProfile(userId)
  const { mutate: addVehicle, isPending: isAdding } = useAddVehicle(userId)
  const { mutate: removeVehicle, isPending: isRemoving, variables: removingId } = useRemoveVehicle(userId)
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        <SavedVehicles
          ...
          isAdding={isAdding}
          removingId={isRemoving ? (removingId ?? null) : null}
