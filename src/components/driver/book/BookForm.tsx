...
export function BookForm({ userId, onSuccess }: ReadonlyBookFormProps) {
  ...
  const { data: availability } = useAvailability(selectedLotId || undefined)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-gray-900" htmlFor="parkingLotId">
            Bãi Đỗ Xe
          </label>
          <div className="relative">
            <select
              ...
              className="w-full appearance-none bg-white border border-gray-300 rounded-lg py-2 pl-3 pr-10 text-sm text-gray-900 focus:ring-2 focus:ring-blue-600 focus:border-blue-600 outline-none"
            >
          ...
          {errors.parkingLotId && (
            <p className="text-sm text-red-600">{errors.parkingLotId.message}</p>
          )}
        </div>
      ...
      {/* Availability */}
      {selectedLotId && selectedVehicleTypeId && (
