...
/** Bảng giá theo giờ — key theo danh sách policyId ở cha để state edit reset khi dữ liệu server đổi. */
function PricingTable({ policies }: PricingTableProps) {
  const updatePricing = useUpdatePricing()
  const [rates, setRates] = useState<Record<string, string>>(() =>
    Object.fromEntries(policies.map((p) => [p.policyId, String(p.hourlyRate)])),
  )
        ...
        return (
          <div key={p.policyId} className="flex items-center gap-4 px-5 py-3">
            ...
            <Button
              ...
              onClick={() => updatePricing.mutate({ policyId: p.policyId, hourlyRate: Number(value) })}
            >
              Lưu
            </Button>
          </div>
